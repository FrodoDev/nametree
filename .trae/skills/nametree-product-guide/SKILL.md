...
- Tree branch vertical order is thought structure, not decoration. Do not reverse, rebalance, or reinterpret sibling branch order to match generic top-to-bottom reading habits.
- For output branches, earlier siblings are more foundational/important and should stay closer to the ground line; later siblings grow upward. This applies to trunk-level branches and branch descendants. If an outline says `RPG` before `MOBA`, `RPG` must keep the earlier/lower branch slot rather than being moved above `MOBA`.
- Trunk-level branch layout protocol: solve in this order and never collapse the steps: (1) preserve the authored sibling order for all trunk-level branches, from lower to higher; (2) preserve each branch's side, including explicit `[left]`/`[right]` and deterministic defaults for unmarked siblings; (3) only then tune visual spacing by moving trunk departure/start heights and branch angles/distances. If order or side is wrong, fix that first before touching spacing.
- Trunk-level branch spacing baseline: only prevent vertical overlap within the same side. Opposite-side branches may occupy nearby vertical bands so the crown stays compact. Main branch length should follow a pronounced natural crown rhythm: lower branches shorter, middle branches longer, upper branches shorter. The length contrast should be visually meaningful, not a tiny offset; when tuning this curve, amplify the middle-vs-edge distance before changing order or side. Add a small asymmetric side offset so left/right main branch departure points do not become perfectly mirrored.
- Use one consistent output-crown model for sparse and dense trees. Tree density may change visual parameters such as horizontal expansion, column count, column spacing, and branch clearance, but it must never change authored sibling order, side assignment, or parent-child structure. Generate trunk-level visual slots from the global sibling order first; then apply side and spacing decisions.
- Do not use dense horizontal staggering of descendant columns as a general solution for many trunk-level branches. Shallow trees do not need it, and deep trees become harder to read because connector lines interleave even when nodes do not overlap. For output layout, prefer clear same-side visual bands over aggressive vertical compression: each same-side trunk-level subtree should remain readable as its own parent-child region. Opposite-side branches may still share nearby vertical bands, and trunk-level branch distance should still follow the lower-short / middle-long / upper-short crown rhythm.
- For dense same-side output crowns, prefer whole-subtree column expansion over depth-based staggering. Move an entire trunk-level subtree outward as a block to reduce overall height, rather than interleaving that subtree's descendant columns with another subtree. This preserves parent-child readability while avoiding a tall fir-tree shape.
- When a later same-side trunk-level branch is placed outward or upward, its trunk-to-branch line should emerge above the previous same-side subtree's node area, with visible clearance, rather than cutting through the previous subtree's labels. It is acceptable to sacrifice some compactness to make sibling order and parent-child regions obvious.
- Trunk-level spacing may only move the current or later branch start upward; never move an earlier trunk branch after it has been placed. Every later trunk-level branch start must remain visually higher than the previous trunk-level branch start, even when same-side collision avoidance shifts a subtree. Do not use subtree-wide post-shifts that make a later branch appear below an earlier branch.
- Layout algorithms must not reorder siblings, rebalance left/right sides, rewrite `tree_edges`, change descendant layout structure, or move child nodes independently to solve trunk-level spacing. For a spacing-only change, the allowed degrees of freedom are trunk-level start height, trunk-level branch angle, and trunk-level horizontal distance.
...
- `[left]` and `[right]` explicitly set side for a node. If omitted on a top-level branch/root, default side assignment may alternate left/right using a fixed deterministic rule across the full sibling list, not just the first few items. Child nodes inherit their parent side unless explicitly changed by future behavior.
- Sibling outline order is semantic order. For `branches:`, every output-side level renders outline order from lower to higher so later siblings grow upward; this applies to the full sibling list at every level, not only the first visible or first tested items. Earlier branch siblings must remain closer to the ground line and must not be moved above later siblings for generic top-to-bottom reading convenience. For `roots:`, keep outline order from upper to lower because roots grow downward.
