...
- Trunk-level branch spacing baseline: only prevent vertical overlap within the same side. Opposite-side branches may occupy nearby vertical bands so the crown stays compact. Main branch length should follow a natural crown rhythm: lower branches shorter, middle branches longer, upper branches shorter. Add a small asymmetric side offset so left/right main branch departure points do not become perfectly mirrored.
